...
@Tag(name = "CallTranscript", description = "통화 transcript API")
public interface CallTranscriptApi {

    @Operation(
            summary = "분석 트리거",
            description = "두 화자의 녹음이 모두 업로드된 통화에 대해 STT 변환 파이프라인을 비동기로 시작한다."
                    + " 이미 처리 중이거나 완료된 경우 기존 transcript 정보를 그대로 반환한다 (멱등)."
    ...
    @Operation(
            summary = "transcript 조회",
            description = "통화의 STT 변환 결과를 조회한다."
                    + " status에 따라 PROCESSING(진행 중) / COMPLETED(segments 포함) / FAILED 응답이 반환된다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200",
