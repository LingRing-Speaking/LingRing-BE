...
public interface UserBlockApi {
    ...
    @Operation(
            summary = "사용자 차단",
            description = "인증된 사용자가 blockedUserId의 사용자를 차단한다. 이미 차단된 경우 기존 차단 정보를 반환한다 (멱등)."
    )
    @ApiResponses({
    ...
    @Operation(
            summary = "사용자 차단 해제",
            description = "인증된 사용자가 blockedUserId 사용자에 대한 차단을 해제한다. 차단 기록이 없어도 204를 반환한다 (멱등)."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "204",
    ...
    @Operation(
            ...
            description = "인증된 사용자가 차단한 목록을 최근 차단순(created_at DESC)으로 반환한다. 무한 스크롤용 Slice. size는 서버에서 최대 50으로 clamp된다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200",
